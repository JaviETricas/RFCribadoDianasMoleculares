...
# -*- coding: utf-8 -*-
"""
descriptor_sin_nm.py — Procesado robusto (sin argumentos) alineado con el CSV de referencia, pero
...
- Filtra a IC50 en CHEMBL332 con unidades distintas de nM (incluye vacías) y relación '=' (normalizada).
...
- Guarda CSV como Dataset/Procesados/descriptores.csv (o Dataset/Prediccion/descriptores.csv con --predict).
"""
...
def filter_like_reference(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Standard Units" in df.columns:
        units = df["Standard Units"].astype(str).str.strip().str.lower()
        cond &= (units != "nm")
    return df[cond].copy()

# ====== Pipeline ======
def process_file(df: pd.DataFrame, smiles_col: str) -> pd.DataFrame:
    # 0) Filtro estilo referencia invertido (IC50, != nM, '=' y CHEMBL332)
